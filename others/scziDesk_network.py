...
class autoencoder(object):
    ...
    def funetrain(self, X, count_X, size_factor, batch_size, funetrain_epoch, update_epoch, error, Y):
        ...
        kmeans_ARI = np.around(adjusted_rand_score(Y, self.kmeans_pred), 5)
        kmeans_NMI = np.around(
            normalized_mutual_info_score(Y, self.kmeans_pred), 5)
        print(f"ARI {kmeans_ARI}, NMI {kmeans_NMI}")
        fune_index = 0
        for i in range(0, funetrain_epoch + 1):
            if i % update_epoch == 0:
                ...
                ARI = np.around(adjusted_rand_score(Y, self.Y_pred), 5)
                NMI = np.around(normalized_mutual_info_score(Y, self.Y_pred), 5)
                print(f"{i} ARI {ARI}, NMI {NMI}")
